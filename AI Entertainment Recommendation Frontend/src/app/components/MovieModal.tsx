...
import { X, Play, Plus, ThumbsUp, Star, Clock, Calendar, Globe, Brain } from 'lucide-react';
import { useApp } from '../context/AppContext';
import { StarRating } from './StarRating';
import { AIScoreBreakdown } from './AIScoreBreakdown';
import { getImageUrl, getBackdropUrl } from '../services/tmdb';
import { getMovieSummary, getMovieReviews } from '../services/backend';
...
export const MovieModal: React.FC = () => {
  const { selectedMovie, setSelectedMovie, userRatings, submitRating } = useApp();
  ...
  // Get similar movies (exclude current)
  const similarMovies = movie
    ? MOCK_MOVIES.filter(m => m.id !== movie.id && m.genres.some(g => movie.genres.includes(g))).slice(0, 4)
    : [];
  ...
  const loadTabData = async (tab: ModalTab) => {
    ...
    if (tab === 'reviews' && !reviews) {
      setIsLoadingData(true);
      const data = await getMovieReviews(movie.tmdbId);
      setReviews(data);
      setIsLoadingData(false);
    }
  };
                  ...
                  <div style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 12, padding: 16 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <span style={{ color: '#ccc', fontSize: '0.8rem' }}>{movie.release_date?.slice(0, 4)}</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span style={{ color: '#666', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: 4 }}>
                          <Clock size={12} /> Runtime
                        </span>
                        <span style={{ color: '#ccc', fontSize: '0.8rem' }}>{movie.runtime}m</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span style={{ color: '#666', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: 4 }}>
                          <Globe size={12} /> Language
                        </span>
                        <span style={{ color: '#ccc', fontSize: '0.8rem', textTransform: 'uppercase' }}>EN</span>
                      </div>
                    </div>
                  </div>
                </div>
